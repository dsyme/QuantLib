...
To date, 99~theorems have been fully machine-checked across eight verification targets
with 9~Lean files, and only 4~remaining proof obligations (3~requiring Float axioms and
1~requiring a derivative composition lemma).
Correspondence between the Lean models and the C++ source is validated through
executable test harnesses across all targets.
Three new high-value targets have been identified for future work:
the Black option pricing formula, matrix algebra, and Newton's method.
...
\begin{enumerate}
  ...
  \item Prove 107~theorems fully and identify 4~proof obligations
...
NormalDistribution & 14 & 1 & positivity, linarith, Real.* \\
Bisection & 11 & 0 & ring, omega, linarith \\
\midrule
\textbf{Total} & \textbf{99} & \textbf{4} & \\
\bottomrule
\end{tabular}
...
\paragraph{Scope.}
Eight of QuantLib's many components have been verified.
The remaining \texttt{sorry} obligations (4 total) require either
...
production quantitative finance library.  Ninety-nine theorems have
been proved across eight targets, covering fundamental algebraic
...
Future work includes completing the remaining 4~\texttt{sorry}
obligations (requiring Float axioms or a derivative composition lemma)
and extending verification to three newly identified high-value targets:
(1)~the \emph{Black option pricing formula}, which composes with the
already-verified NormalDistribution and offers canonical put-call parity
proofs; (2)~\emph{matrix operations}, with rich algebraic properties
(commutativity, associativity, transpose involution); and
(3)~\emph{NewtonSafe}, a hybrid Newton/bisection solver that extends
the successfully verified Bisection proof patterns.
A floating-point closeness comparison utility (\texttt{close}/\texttt{close\_enough})
is also in progress at the informal specification stage.
