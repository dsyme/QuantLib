...
conventions, interest rate compounding algebra, option pricing formulas, numerical
solvers, matrix operations, rounding, and number-theoretic utilities---as pure
functional definitions over exact arithmetic and prove key algebraic properties
including additivity, round-trip inversion, monotonicity, convergence bounds,
put-call parity, and structural correctness.
To date, 291~theorems have been fully machine-checked across 17~verification targets
in 18~Lean files, with only 5~remaining proof obligations requiring Float axioms
or advanced analysis lemmas.
Correspondence between the Lean models and the C++ source is validated through
executable test harnesses covering 15~targets with thousands of test cases.
...
\begin{enumerate}
  ...
  \item Prove 291~theorems fully and identify 5~proof obligations
        requiring advanced tooling (\S\ref{sec:results}).
  \item Validate model correspondence via executable
        test cases comparing Lean models against C++
        implementations across 15~targets (\S\ref{sec:correspondence}).
\end{enumerate}
...
Eight candidates were identified and verified to varying degrees
(Table~\ref{tab:targets}).  The project has since expanded to 17~targets
covering day counting, interest rates, numerical methods, option pricing,
matrix algebra, rounding, and number-theoretic utilities.
...
Actual365Fixed & High & Easy & 8 & {\checkmark} Complete \\
InterestRate & High & Moderate & 30 & {\checkmark} Complete$^*$ \\
...
NormalDistribution & Medium & Hard & 20 & Partial (1 sorry) \\
Bisection & Med--High & Hard & 15 & {\checkmark} Complete \\
FloatingPointClose & High & Moderate & 12 & {\checkmark} Complete \\
BlackFormula & High & Hard & 14 & Partial (1 sorry) \\
NewtonSafe & Med--High & Moderate & 13 & {\checkmark} Complete \\
Matrix & High & Moderate & 23 & {\checkmark} Complete \\
PlainVanillaPayoff & High & Easy & 20 & {\checkmark} Complete \\
Quadratic & High & Moderate & 17 & {\checkmark} Complete \\
Composition & High & Hard & 28 & {\checkmark} Complete \\
Rounding & High & Moderate & 21 & {\checkmark} Complete \\
PrimeNumbers & High & Moderate & 14 & {\checkmark} Complete \\
BernsteinPolynomial & High & Moderate & --- & Spec phase \\
\midrule
\textbf{Total} & & & \textbf{291} & \textbf{5 sorry} \\
\bottomrule
\end{tabular}
...
Correspondence validation now covers 15~targets: Actual360, Actual365Fixed,
Bisection, BlackFormula, Factorial, FloatingPointClose, InterestRate,
LinearInterpolation, Matrix, NewtonSafe, NormalDistribution,
PlainVanillaPayoff, Quadratic, Rounding, and Thirty360.
Test harnesses range from curated point cases to exhaustive sweeps,
with total case counts exceeding 8{,}000.  No mismatches have been observed.
FloatingPointClose uses 1{,}696~test cases validating the comparison semantics.
PlainVanillaPayoff uses 823~cases covering call/put payoff computation.
Quadratic uses 63~cases covering all discriminant regimes.
...
Thirty theorems are proved across three models (Rat, Real, Float):
...
and their derivatives. Fourteen theorems are proved: PDF positivity,
...
\subsection{Actual365Fixed Day Counter}

The Actual365Fixed day counter computes year fractions as
$\mathit{dayCount}(d_1, d_2) / 365$, using the standard convention.
Eight theorems are fully proved: formula correctness, non-negativity,
positivity with includeLastDay, additivity, same-date identity,
antisymmetry, includeLastDay characterisation, and linear scaling.
All proofs close with \texttt{simp} + \texttt{omega}, mirroring
the Actual360 proof structure.

\subsection{Bisection Root-Finding}

The bisection algorithm is modelled with fuel-bounded recursion over
exact rationals. Eleven theorems are fully proved: interval
halving per step, interval width after $k$ steps ($|dx|/2^k$),
midpoint containment, orientation correctness, root-in-interval
invariant, convergence bounds, termination, and accuracy guarantees.
The proofs use \texttt{ring}, \texttt{omega}, and \texttt{linarith}.
...
Actual365Fixed & 8 & 0 & omega, simp \\
InterestRate & 30 & 3 & ring, positivity, gcongr \\
Thirty360 & 11 & 0 & omega, simp, ring \\
LinearInterpolation & 7 & 0 & ring, simp, linarith \\
Factorial & 10 & 0 & omega, induction, simp \\
NormalDistribution & 20 & 1 & positivity, linarith, Real.* \\
Bisection & 15 & 0 & ring, omega, linarith \\
FloatingPointClose & 12 & 0 & simp, omega, decide \\
BlackFormula & 14 & 1 & ring, positivity, linarith \\
NewtonSafe & 13 & 0 & ring, omega, linarith \\
Matrix & 23 & 0 & ring, simp, ext \\
PlainVanillaPayoff & 20 & 0 & simp, omega, ring \\
Quadratic & 17 & 0 & ring, omega, field\_simp \\
Composition & 28 & 0 & ring, simp, linarith \\
Rounding & 20 & 0 & omega, simp, decide \\
PrimeNumbers & 14 & 0 & decide, Nat.Prime, omega \\
\midrule
\textbf{Total} & \textbf{291} & \textbf{5} & \\
\bottomrule
\end{tabular}
...
${\sim}65$ lines.  Actual365Fixed follows the same pattern with
8~theorems constraining a similar-sized C++ implementation.
The InterestRate specification is 30~proved
theorems (${\sim}80$ lines of Lean) constraining ${\sim}360$
lines of C++ spanning five compounding modes.  The Bisection
specification is 15~theorems constraining a ${\sim}90$-line C++
implementation.  In all cases, the algebraic laws are
...
\paragraph{Scope.}
Seventeen of QuantLib's many components have been verified.
The remaining 5~\texttt{sorry} obligations require either
Float axioms (3 in InterestRate) or advanced Mathlib lemmas
(1 NormalDistribution \texttt{HasDerivAt}, 1 BlackFormula $\Phi$ axiom)
that Lean's automation cannot yet close without manual proof engineering.
...
production quantitative finance library.  Two hundred and ninety-one
theorems have been proved across 17~targets, covering fundamental
algebraic properties of day counting, interest rate compounding,
interpolation, probability distributions, combinatorics, root-finding,
option pricing, matrix algebra, rounding, and number theory.
Correspondence testing across 15~targets validates model fidelity.
No implementation bugs were found, providing formal assurance
of correctness for these well-established components.

Future work includes completing the remaining 5~\texttt{sorry}
obligations (requiring Float axioms or analysis lemmas),
formalising new targets (BernsteinPolynomial, RichardsonExtrapolation,
LagrangeInterpolation), extending correspondence testing to the
remaining targets (Composition, PrimeNumbers), and developing
cross-target composition theorems for realistic financial workflows.
